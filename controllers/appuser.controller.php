<?php class AppuserController extends DatabaseController
{
    ...
    // Cette méthode va permettre de récupérer les données dans les tables account et role,
    // elle sera appelée dans les méthode getAllWith et getOneWith
    ...
    public function affectDataToRow(&$row, $sub_rows)
    {
        ...
        if (isset($sub_rows['article'])) {
            $articles = array_filter($sub_rows['article'], function ($item) use ($row) {
                return $item->Id_appUser == $row->Id_appUser;
            });
            if (isset($articles)) {
